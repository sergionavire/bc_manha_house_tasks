...
export function WorkDayUpdate() {
  ...
  useEffect(() => {
    apiHousetasks.get(`/work_days/${params.id}`).then((item) => {
      const workDay: WorkDayType = item.data[0];
      setHouseId(workDay.house_id);
      setUsersIdWorker(workDay.users_id_worker);
      ...
      <form
        ...
        onSubmit={async (event) => {
          ...
          if (updateWorkDayResponse.success) {
            ...
            navigate(`/workday-view/${params.id}`);
          } else {
